...
start_patching("AsiaPayClientPost");

$location = "skins/default/en/checkout/details_dialog.tpl";

if (!is_template_patched($location, "AsiaPayClientPost"))
{
	$find_str = <<<EOT
<!-- PAYMENT METHOD FORM -->
EOT;
    $replace_str = <<<EOT
<widget module="AsiaPayClientPost" template="modules/AsiaPayClientPost/checkout.tpl" visible="{cart.paymentMethod.payment_method=#asiapayclientpost#}">
<!-- PAYMENT METHOD FORM -->
EOT;
    patch_template($location, "AsiaPayClientPost", $find_str, $replace_str);
}
else
{
